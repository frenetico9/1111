...
import { DAYS_OF_WEEK, CORTE_CERTO_ID } from '../constants';
...
const HomePage: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const profilePromise = mockGetBarbershopProfile(CORTE_CERTO_ID);
      const servicesPromise = mockGetServicesForBarbershop(CORTE_CERTO_ID);
      const reviewsPromise = mockGetReviewsForBarbershop(CORTE_CERTO_ID);
        ...
        <div className="grid md:grid-cols-12 gap-8">
            ...
            {services.length > 0 && reviews.length === 0 && (
                <div>
                    ...
                    <p>Nossa barbearia ainda não possui avaliações. Seja o primeiro a avaliar após seu agendamento!</p>
                    </div>
                </div>
            )}
            </section>
