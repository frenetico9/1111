...
const BookingPage: React.FC = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const createdAppointment = await mockCreateAppointment(newAppointmentData);
      addNotification({ message: 'Agendamento realizado com sucesso!', type: 'success' });
      setLastAppointment(createdAppointment);
  ...
  const handleRecurrenceSelection = async (weeks: number) => {
    if (!lastAppointment) return;
    setIsCreatingRecurring(true);
    try {
        await mockCreateFutureAppointment(lastAppointment, weeks);
        addNotification({ message: `Novo agendamento criado para daqui a ${weeks} semanas!`, type: 'success' });
    } catch (error) {
        addNotification({ message: `Não foi possível agendar: ${(error as Error).message}`, type: 'error' });
    } finally {
  ...
  const closeRecurrenceModalAndRedirect = () => {
      setShowRecurrenceModal(false);
      navigate('/client/appointments');
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-6">
      ...
      <Modal isOpen={showRecurrenceModal} onClose={closeRecurrenceModalAndRedirect} title="Agendamento Recorrente">
        <div className="text-center">
            <h3 className="text-lg font-semibold text-text-dark">Deseja repetir este agendamento?</h3>
            <p className="text-sm text-text-light mt-2 mb-6">Economize tempo e garanta seu horário com antecedência.</p>
            <div className="space-y-3">
                 <Button onClick={() => handleRecurrenceSelection(2)} fullWidth isLoading={isCreatingRecurring} disabled={isCreatingRecurring}>A cada 2 semanas</Button>
                 <Button onClick={() => handleRecurrenceSelection(4)} fullWidth isLoading={isCreatingRecurring} disabled={isCreatingRecurring}>A cada 4 semanas</Button>
                 <Button onClick={closeRecurrenceModalAndRedirect} fullWidth variant="secondary" disabled={isCreatingRecurring}>Não, obrigado</Button>
            </div>
        </div>
      </Modal>
    </div>
